...
interface Env extends CloudflareEnv {
  ...
  AUTO_PODCAST_GENERATOR_R2_BUCKET_URL: string
  ...
  async run(event: WorkflowEvent<Params>, step: WorkflowStep) {
    ...
    for (const story of stories) {
      ...
      await step.do(`store story ${story.id} summary`, retryConfig, async () => {
        ...
        await this.env.AUTO_PODCAST_GENERATOR_KV.put(storyKey, `<story>${text}</story>`, { expirationTtl: 3600 })
        return storyKey
      })
    ...
    const allStories = await step.do('collect all story summaries', retryConfig, async () => {
      ...
      for (const story of stories) {
        const storyKey = `tmp:${event.instanceId}:story:${story.id}`
        const summary = await this.env.AUTO_PODCAST_GENERATOR_KV.get(storyKey)
        if (summary) {
    ...
    for (const [index, conversation] of conversations.entries()) {
      await step.do(`create audio ${index}: ${conversation.substring(0, 20)}...`, { ...retryConfig, timeout: '5 minutes' }, async () => {
        ...
        const audioUrl = `${this.env.AUTO_PODCAST_GENERATOR_R2_BUCKET_URL}/${audioKey}?t=${Date.now()}`

        await this.env.AUTO_PODCAST_GENERATOR_R2.put(audioKey, audio)

        this.env.AUTO_PODCAST_GENERATOR_KV.put(`tmp:${event.instanceId}:audio:${index}`, audioUrl, { expirationTtl: 3600 })
        return audioUrl
      })
    }
    ...
    const audioFiles = await step.do('collect all audio files', retryConfig, async () => {
      ...
      for (const [index] of conversations.entries()) {
        const audioUrl = await this.env.AUTO_PODCAST_GENERATOR_KV.get(`tmp:${event.instanceId}:audio:${index}`)
        if (audioUrl) {
    ...
    await step.do('concat audio files', retryConfig, async () => {
      ...
      const blob = await concatAudioFiles(audioFiles, this.env.BROWSER, { workerUrl: this.env.AUTO_PODCAST_GENERATOR_WORKER_URL })
      await this.env.AUTO_PODCAST_GENERATOR_R2.put(podcastKey, blob)

      const podcastAudioUrl = `${this.env.AUTO_PODCAST_GENERATOR_R2_BUCKET_URL}/${podcastKey}?t=${Date.now()}`
    ...
    await step.do('save content to kv', retryConfig, async () => {
      await this.env.AUTO_PODCAST_GENERATOR_KV.put(contentKey, JSON.stringify({
        date: today,
    ...
    await step.do('clean up temporary data', retryConfig, async () => {
      ...
      for (const story of stories) {
        const storyKey = `tmp:${event.instanceId}:story:${story.id}`
        deletePromises.push(this.env.AUTO_PODCAST_GENERATOR_KV.delete(storyKey))
      }

      // Clean up audio temporary data
      for (const [index] of conversations.entries()) {
        const audioKey = `tmp:${event.instanceId}:audio:${index}`
        deletePromises.push(this.env.AUTO_PODCAST_GENERATOR_KV.delete(audioKey))
      }
      ...
      for (const index of audioFiles.keys()) {
        try {
          await Promise.any([
            this.env.AUTO_PODCAST_GENERATOR_R2.delete(`tmp/${podcastKey}-${index}.mp3`),
